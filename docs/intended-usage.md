...
This page explains how iugo-ai is meant to be used. Not the flags, not the architecture -- just the mental model. If you read one page besides the README, make it this one.

---

## After Installing -- You're Done

Once you run `iugo-ai` and select your agent(s), components, and preset, everything is configured. There is nothing else to do. No commands to memorize, no workflows to learn, no config files to edit.
...
For full documentation: [github.com/IUGO-Programming/engram](https://github.com/IUGO-Programming/engram)
...
2. Create a profile via iugo-ai TUI ("OpenCode SDD Profiles") or CLI (`--profile` flag)
3. The base/default SDD conductor is `iugo-orchestrator`
4. Named profiles generate `sdd-orchestrator-{name}` + suffixed sub-agents, each assigned to your chosen model
5. In OpenCode, press **Tab** to switch between `iugo-orchestrator` and custom profiles

You can create multiple profiles (e.g., "cheap" for experimentation, "premium" for production) and switch between them freely.

If you prefer a **runtime profile manager** that keeps profiles outside `opencode.json`, iugo-ai now supports that too. During sync, OpenCode can auto-detect external profile files under `~/.config/opencode/profiles/*.json` and switch to a safer compatibility path that preserves the active `iugo-orchestrator` prompt instead of overwriting it.
...
iugo-ai installs **SDD skills** and **foundation skills** (workflow, testing patterns) directly into your agent's skills directory. These are embedded in the binary and always up to date.

For **coding skills** (React 19, Angular, TypeScript, Tailwind, Zod, Playwright, etc.), the community maintains a separate repository: [IUGO-Programming/IUGO-Skills](https://github.com/IUGO-Programming/IUGO-Skills). You install those manually by cloning the repo and copying the skills you want:

```bash
git clone https://github.com/IUGO-Programming/IUGO-Skills.git
cp -r IUGO-Skills/curated/react-19 ~/.claude/skills/
cp -r IUGO-Skills/curated/typescript ~/.claude/skills/
...
1. **The registry refreshes at startup where the agent supports hooks.** Normal Pi startup runs the `iugo-pi` session hook. Codex, Claude Code, and OpenCode run `iugo-ai skill-registry refresh --quiet` from their installed startup/plugin hooks.
2. **The refresh is cached.** IUGO-AI fingerprints discovered `SKILL.md` files using schema version, path, mtime, and size. If `.atl/.skill-registry.cache.json` matches and `.atl/skill-registry.md` exists, startup is a cheap cache-hit.
...
4. **Manual fallback stays available** -- run `iugo-ai skill-registry refresh --force` from a project if you want to regenerate immediately.
...
The less you think about iugo-ai after installing, the better it's working.
...
| Let startup hooks refresh the skill registry automatically | Manually rescan skills unless you need `iugo-ai skill-registry refresh --force` |
