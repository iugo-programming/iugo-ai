...
|---|---|
| `cmd/iugo-ai/main.go` | Binary entrypoint and version handoff. |
...
| `internal/state/state.go` | Persisted install state in `~/.iugo-ai/state.json`. |
...
| Engram store schema | Sessions, observations, prompts, relations, and sync mutations are external Engram runtime concepts, not IUGO-AI source files. |
