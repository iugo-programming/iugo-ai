...
IUGO-AI integration code should stay thin: adapters describe where and how an agent accepts configuration; components decide what managed content to inject.
...
OpenCode community plugins are optional integrations. IUGO-AI only ensures `~/.config/opencode/tui.json` exists and contains the plugin package name. It does not vendor, execute, or own plugin runtime code.
