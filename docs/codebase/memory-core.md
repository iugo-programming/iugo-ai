...
IUGO-AI wires Engram into agents; Engram owns the memory store. This page explains the boundary so maintainers do not confuse installer code with memory database code.
...
IUGO-AI docs and prompt assets refer to these Engram concepts, but their schema is not defined in this repo.
...
AI agent receives prompt
  |
  v
IUGO-AI-installed prompt tells agent to use Engram MCP tools
  |
  v
Agent calls `engram mcp --tools=agent` via configured MCP entry
  ...
  |
  v
Engram runtime stores and searches memory outside iugo-ai source
```
...
- [ ] Decide whether the change belongs to IUGO-AI wiring or Engram itself.
