...
End-to-end tests that validate the `iugo-ai` installer binary inside Docker containers running real Linux distributions.
...
docker build -f e2e/Dockerfile.ubuntu -t iugo-ai-e2e-ubuntu .
docker run --rm iugo-ai-e2e-ubuntu

# Run with full E2E on Arch
docker build -f e2e/Dockerfile.arch -t iugo-ai-e2e-arch .
docker run --rm -e RUN_FULL_E2E=1 iugo-ai-e2e-arch

# Interactive debugging
docker run --rm -it iugo-ai-e2e-ubuntu /bin/bash
```
...
4. Build the `iugo-ai` binary for `linux/amd64`
