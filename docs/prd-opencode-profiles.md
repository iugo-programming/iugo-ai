...
**R-PROF-05**: El nombre "default" ESTÁ RESERVADO para el conductor SDD base de OpenCode (`iugo-orchestrator`). El usuario NO puede crear un perfil llamado "default".
...
**R-PROF-07b**: El perfil `default` también se PUEDE editar — es el `iugo-orchestrator` base. Editar el default es equivalente a lo que hoy hace "Configure Models → OpenCode" pero integrado en el flujo de perfiles.
...
**R-PROF-23**: El prompt del orchestrator (`orchestrator.md`) DEBE incluir un bloque `<!-- iugo-ai:sdd-model-assignments -->` que se inyecta dinámicamente con la tabla de modelos de ESE perfil específico.
...
**R-PROF-43**: El perfil default (`iugo-orchestrator`) SIEMPRE existe cuando SDD está configurado. Los perfiles adicionales son opcionales.
...
5. **¿El `iugo-agent` agent (persona) también necesita variantes por perfil?**
   → No. El `iugo-agent` agent es la persona general, no parte de SDD. El conductor SDD base de OpenCode es `iugo-orchestrator`.
