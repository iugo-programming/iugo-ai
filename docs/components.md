...
|-----------|-----|-------------|
| Engram | `engram` | Persistent cross-session memory via MCP — auto-detection of project name, full-text search, git sync, project consolidation. See [engram repo](https://github.com/IUGO-Programming/engram) |
...
| Persona | `persona` | Managed IUGO/neutral persona injection, or unmanaged custom persona mode |
...
| GGA | `gga` | IUGO Guardian Angel — AI provider switcher |
| Theme | `theme` | IUGO Kanagawa theme overlay |

## GGA Behavior

`iugo-ai install --component gga` installs/provisions the `gga` binary globally on your machine.
...
### Included Skills (installed by iugo-ai)
...
These foundation skills are installed by default with both the `full-iugo-agent` (Dev Stack + Polish) and `ecosystem-only` (Dev Stack) presets.

### Coding Skills (separate repository)

For framework-specific skills (React 19, Angular, TypeScript, Tailwind 4, Zod 4, Playwright, etc.), see [IUGO-Programming/IUGO-Skills](https://github.com/IUGO-Programming/IUGO-Skills). These are maintained by the community and installed separately by cloning the repo and copying skills to your agent's skills directory.
...
|--------|-----|-----------------|
| Dev Stack + Polish | `full-iugo-agent` | All components (Engram + SDD + Skills + Context7 + GGA + Permissions + Theme) + all skills |
