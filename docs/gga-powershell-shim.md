...
When `iugo-ai` installs GGA on Windows, it now installs a `gga.ps1` wrapper
...
check — re-running `iugo-ai install` is idempotent.
...
`iugo-ai install`**. In the final design this was moved to **runtime** — the `.ps1`
