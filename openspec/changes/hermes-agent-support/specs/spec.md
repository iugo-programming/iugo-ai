...
The adapter MUST report `SupportsAutoInstall() = false`. Hermes is detect-only; iugo-ai will never attempt to install it automatically.
...
`InstallCommand()` MUST return an error indicating Hermes is not auto-installable, with a message directing the user to install Hermes manually. It MUST NOT return a command slice that iugo-ai will execute.
...
`SystemPromptStrategy()` MUST return `model.StrategyMarkdownSections`. SOUL.md content is managed via `<!-- iugo-ai:... -->` marker-based section injection without replacing user content.
...
When iugo-ai configures Hermes, the `context7` MCP server block MUST be written to `~/.hermes/config.yaml` under `mcp_servers:`.
...
When iugo-ai configures Hermes with engram enabled, the `engram` MCP server block MUST be written to `~/.hermes/config.yaml` under `mcp_servers:`.
...
- WHEN iugo-ai reconfigures Hermes (engram injection runs again)
...
- WHEN iugo-ai reconfigures Hermes
...
When `sdd.Inject()` is called with the Hermes adapter, the SDD orchestrator content MUST be written into `~/.hermes/SOUL.md` using `<!-- iugo-ai:sdd-orchestrator -->` markers, without replacing content outside the markers.
...
- THEN `~/.hermes/SOUL.md` is created and contains `<!-- iugo-ai:sdd-orchestrator -->` markers with orchestrator content between them
...
- THEN user content outside the `<!-- iugo-ai:sdd-orchestrator -->` markers is preserved verbatim
- AND the orchestrator block is inserted or updated within the markers

#### Scenario: Orchestrator injection — idempotent

- GIVEN `~/.hermes/SOUL.md` already contains the `<!-- iugo-ai:sdd-orchestrator -->` block
...
The strict-TDD instructions MUST be injected into `~/.hermes/SOUL.md` via `<!-- iugo-ai:strict-tdd -->` markers using the standard markdown-sections flow.
...
### Requirement: Hermes-Specific Persona Asset for IUGO Options

For the `iugo-agent` and `iugo-agent-neutral-artifacts` persona options, `personaContent()` MUST return the content of a dedicated Hermes asset (e.g., `hermes/persona-iugo-agent.md`), not the generic `generic/persona-iugo-agent.md`.
...
#### Scenario: IUGO persona uses Hermes-specific asset

- GIVEN persona option is `iugo-agent` or `iugo-agent-neutral-artifacts`
- WHEN `personaContent(model.AgentHermes, personaOption)` is called
- THEN it returns content from `hermes/persona-iugo-agent.md`
- AND the content does NOT contain `<available_skills>` as an injection mechanism

#### Scenario: Generic asset not used for Hermes iugo-agent

- GIVEN persona option is `iugo-agent`
- WHEN `personaContent(model.AgentHermes, ...)` is called
- THEN it does NOT return content from `generic/persona-iugo-agent.md`
...
When persona injection runs for Hermes, the persona content MUST be written into `~/.hermes/SOUL.md` using the standard `StrategyMarkdownSections` flow (same `<!-- iugo-ai:persona -->` markers).

#### Scenario: Persona injected into SOUL.md

- GIVEN persona option is `iugo-agent` and Hermes adapter is selected
- WHEN persona injection runs
- THEN `~/.hermes/SOUL.md` contains the Hermes-specific iugo-agent persona content within `<!-- iugo-ai:persona -->` markers

### Requirement: Persona Language Contract — Technical Artifacts in English

Technical artifact content (code comments, SOUL.md injected blocks, skill references, identifiers) generated by iugo-ai for Hermes MUST default to English, regardless of persona voice or conversation language. The persona governs agent tone in SOUL.md replies, not the language of embedded technical content.
...
- Hermes with `iugo-agent` option uses `hermes/persona-iugo-agent.md`
