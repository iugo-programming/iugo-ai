...
- The generic persona assets — BOTH `generic/persona-iugo-agent.md` AND `generic/persona-neutral.md` —
...
- Inject context7 + engram MCP into `~/.hermes/config.yaml` idempotently, without destroying
  user content or comments outside iugo-ai-managed server blocks.
- Inject engram-protocol / SDD-orchestrator / strict-TDD into `~/.hermes/SOUL.md` via markdown markers.
- Inject the correct Hermes-specific persona (iugo-agent AND neutral) with the skill-loading block
...
| **Rationale** | Marker sections are battle-tested (Claude Code, OpenClaw). engram, SDD, strict-TDD, and persona each inject their own `<!-- iugo-ai:ID -->` section. Global path = standard flow. |
...
### Decision 5 — Persona Option B: dedicated Hermes assets for BOTH iugo-agent AND neutral
...
affects BOTH personas, not just iugo-agent. Today `personaContent()` (`persona/inject.go`) handles
`PersonaNeutral` with a single non-per-agent `assets.MustRead("generic/persona-neutral.md")` and
has NO per-agent neutral switch — iugo-agent is the only per-agent branch.

| | |
|---|---|
| **Choice** | Create TWO Hermes persona assets and refactor `personaContent()` to support per-agent neutral: <br>• `internal/assets/hermes/persona-iugo-agent.md` (copy of generic iugo-agent, skill-loading block rewritten) <br>• `internal/assets/hermes/persona-neutral.md` (copy of generic neutral, skill-loading block rewritten) |
...
Change the `PersonaNeutral` case to a per-agent inner switch (mirroring the iugo-agent branch),
and add a Hermes case to the iugo-agent branch:
...
```go
		// inside the iugo-agent (default) branch's inner agent switch:
		case model.AgentHermes:
...
|---|---|---|
| `iugo-agent` | `hermes/persona-iugo-agent.md` | `isIUGOConversationPersona` = true |
| `iugo-agent-neutral-artifacts` | `hermes/persona-iugo-agent.md` | same iugo-agent branch (falls into default) |
| `neutral` | `hermes/persona-neutral.md` | NEW per-agent neutral path |
| `custom` | `""` (no-op) | unchanged |

Hermes uses `StrategyMarkdownSections`, so persona is injected as a `<!-- iugo-ai:persona -->`
...
| | |
|---|---|
| **Choice** | Document the complementary relationship inside the **Hermes persona asset** (`hermes/persona-iugo-agent.md` and `hermes/persona-neutral.md`) as a short subsection, NOT in the shared `claude/engram-protocol.md`. |
| **Alternatives** | (a) Add it to `claude/engram-protocol.md` — rejected: that asset is shared by every agent; Hermes-specific text would leak into all of them. (b) A separate `<!-- iugo-ai:hermes-memory-note -->` marker section injected by the engram component — rejected: adds a Hermes-only branch to the engram injector for one paragraph; the persona asset is already Hermes-specific and always present. |
...
iugo-ai-managed block, re-append a fresh block, return content ending in a single trailing `\n`.
...
// enabling iugo-ai to preserve a command already written for a server
...
- Content **outside** any iugo-ai-managed server block is preserved verbatim, including:
  user comments (`# ...`), other top-level keys, and other MCP servers (user-defined or a sibling
  iugo-ai server).
- Comments **inside** a managed server block are iugo-ai-owned and may be lost on re-write
...
| `internal/assets/hermes/` | NEW: `persona-iugo-agent.md`, `persona-neutral.md`, `sdd-orchestrator.md` |
| `internal/components/sdd/inject.go` | `case model.AgentHermes: return "hermes/sdd-orchestrator.md"` in `sddOrchestratorAsset()` |
| `internal/components/persona/inject.go` | per-agent neutral switch + `AgentHermes` iugo-agent case (Decision 5) |
...
Pipeline (all paths use ~/.hermes/ — global, no workspace dir):
  ├── persona:  inject <!-- iugo-ai:persona --> into SOUL.md
  │             (hermes/persona-iugo-agent.md OR hermes/persona-neutral.md)
...
| Comment loss inside managed blocks | Low | Documented: managed blocks are iugo-ai-owned; content outside preserved (golden #6) |
...
| Unit | `personaContent()` — iugo-agent/neutral/iugo-agent-neutral-artifacts/custom for Hermes | Table-driven; assert Hermes assets selected and skill-loading block rewritten |
...
| Integration | persona inject into SOUL.md | `t.TempDir()`; assert `<!-- iugo-ai:persona -->` section; assert engram/SDD sections coexist without duplication |
