...
Neutral should provide IUGO-equivalent mentor behavior without Rioplatense/regional voice. Today neutral avoids most direct IUGO voice leakage, but has a weaker behavior contract and legacy sync can reactivate IUGO when persona state is missing or invalid.
...
- Fix sync defaults so missing/invalid persisted persona does not silently fall back to IUGO.

### Out of Scope
- Changing IUGO regional voice or mentor contract.
- Reworking OpenCode/Kilocode residual `agent.iugo-agent` behavior unless needed to prevent neutral regressions.
...
| OpenCode/Kilocode sync assets | Investigate | Confirm residual `agent.iugo-agent` behavior is intentional and not a neutral regression. |

## Risks

| Risk | Likelihood | Mitigation |
|------|------------|------------|
| Neutral gains regional/IUGO voice | Medium | Add neutral voice constraints and regression coverage. |
| Legacy users lose expected IUGO default | Medium | Define fallback semantics clearly; preserve explicit IUGO selections. |
...
- [ ] Neutral receives IUGO-equivalent behavior rules without regional voice.
- [ ] Claude neutral has a non-default output-style contract.
- [ ] Kimi neutral injected `output-style.md` is not empty.
- [ ] Missing/invalid persisted persona no longer silently reactivates IUGO.
- [ ] Explicit IUGO selection continues to work.
