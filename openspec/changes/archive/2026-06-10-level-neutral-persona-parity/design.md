...
Make `neutral` a first-class twin of `iugo-agent`: the same mentor/verification/response-length contract, but with neutral professional language and no Rioplatense/regional speech rules. Persona injection remains centralized in `internal/components/persona/inject.go`; assets carry the behavioral contract; sync resolves unsafe or missing persisted state to neutral rather than reviving IUGO.
...
| Sync fallback | `applyResolvedPersona` keeps explicit `selection.Persona`; valid persisted values are honored; missing or invalid persisted persona resolves to `PersonaNeutral`. | Continue missing/invalid fallback to IUGO; fail sync on invalid state. | Missing/invalid state is not an explicit IUGO selection. Neutral is safer because it avoids regional voice and surprise persona reactivation while preserving explicit IUGO installs. |
| OpenCode/Kilocode residuals | Keep IUGO agent overlay install-only for merge safety, but allow sync-managed non-IUGO cleanup to remove only `agent.iugo-agent` while preserving other `agent` children. | Never touch residuals during sync; fully manage the overlay during sync. | Narrow cleanup prevents neutral regressions without reintroducing the SDD `agent` clobbering risk documented in `InjectForSync`. |
...
Selection/persona state ──→ applyResolvedPersona ──→ persona.Inject/InjectForSync
                                      ...
                                      └─→ OpenCode/Kilocode cleanup ──────→ remove agent.iugo-agent only
```
...
|---|---|---|
| `internal/assets/generic/persona-neutral.md` | Modify | Bring neutral rules/behavior to IUGO parity: short answers, one question, no option-menu default, verification-first, artifact language boundaries. |
...
| `internal/components/persona/inject.go` | Modify later | Select neutral output-style assets, write Claude `neutral.md`, set/clean managed `outputStyle` values, populate Kimi output-style module, and permit sync cleanup of OpenCode/Kilocode `agent.iugo-agent`. |
...
Managed Claude output-style names are `IUGO` and `Neutral`; cleanup must remove only managed values, not arbitrary user output styles.
...
| Unit | Claude neutral writes `neutral.md`, sets `outputStyle: "Neutral"`, removes stale IUGO style/settings, and is idempotent. | Update `internal/components/persona/inject_test.go`; replace the old “neutral does not write output style” expectation. |
| Unit | Kimi neutral writes non-empty `.kimi/output-style.md` and still bootstraps `KIMI.md`. | Add Kimi neutral injection test beside existing IUGO test. |
| Unit | Missing and invalid persisted persona resolve to neutral; explicit `PersonaIUGO`, `PersonaNeutral`, and `PersonaCustom` still win. | Update `internal/cli/sync_test.go` fallback cases. |
...
No data migration required. On next sync, old state without persona or with invalid persona will resolve to neutral; explicit persisted `iugo-agent` remains IUGO. Rollback is file-level: revert assets and fallback logic.
