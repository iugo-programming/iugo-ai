...
Make the `chained-pr` skill (registry name `chained-pr`, frontmatter `iugo-ai-chained-pr`) the operational source of truth for PR chaining during SDD, by binding it into every SDD orchestrator asset so that `sdd-tasks` and `sdd-apply` sub-agents are required to load and follow it before planning or creating any chained/stacked PR.
