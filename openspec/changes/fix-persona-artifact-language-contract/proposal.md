...
Separate language behavior across three domains so persona style no longer leaks into technical artifacts or public comments. IUGO remains a Rioplatense teaching voice for direct conversation, while generated artifacts default to English and comments react to their target context.

## Product language contract

| Domain | Default contract | Regional Spanish behavior |
|--------|------------------|---------------------------|
| Direct user/orchestrator conversation | Governed by the active persona. | `iugo-agent` uses the expected Rioplatense senior-architect teaching voice: voseo, concepts before code, warm/direct tone. `neutral` keeps the same teaching core without regional tone. |
...
- Preserve IUGO persona assets that intentionally govern direct conversation, while making their artifact-boundary rules clearer if needed.
...
- Removing or weakening the IUGO direct conversation voice.
...
5. **Add regression tests.**
   - Asset-level language guards with allowlists for IUGO direct-conversation persona files.
...
| `internal/components/persona` | Possible tests or wording updates to preserve direct IUGO conversation while preventing artifact leakage. |
...
| Regression accidentally removes IUGO direct voseo. | Allow regional wording only in IUGO direct-conversation persona/output-style assets and add/keep assertions that IUGO Spanish replies remain Rioplatense. |
...
- [ ] Direct conversation remains persona-governed: IUGO keeps Rioplatense teaching voice, neutral keeps the same teaching core without regional tone.
