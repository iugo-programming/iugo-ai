...
**Date**: 2026-04-03
**Status**: Draft
**Parent PRD**: [PRD.md](PRD.md) (IUGO AI Installer)

---

## 1. Problem Statement

The IUGO AI ecosystem ships with a powerful set of pre-built skills and SDD phases. But every developer works differently. A frontend architect needs a design system reviewer. A security engineer needs a vulnerability scanner agent. A technical writer needs a documentation generator.
...
- **Ecosystem-native**: the generated agent has access to Engram (persistent memory), MCP servers, and follows the IUGO skill format
...
```
You are an expert AI skill creator for the IUGO AI ecosystem.
...
```
~/.config/iugo-ai/custom-agents.json
```
...
graph TB
    ...
    subgraph STORAGE["Storage"]
        ...
        REGISTRY_FILE["~/.config/iugo-ai/custom-agents.json"]
...
| R-AB-08 | The Agent Builder MUST maintain a local registry of custom agents at `~/.config/iugo-ai/custom-agents.json` | P0 |
