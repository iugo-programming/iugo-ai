...
// Example: <!-- iugo-ai:custom-agent:my-skill -->
const markerFormat = "<!-- iugo-ai:custom-agent:%s -->"
...
func buildSDDBlock(agent *GeneratedAgent, marker string) string {
	...
	endMarker := fmt.Sprintf("<!-- /iugo-ai:custom-agent:%s -->", agent.Name)
	return marker + "\n" + body + endMarker
}
...
func replaceBlock(content, marker, newBlock string) string {
	endMarker := fmt.Sprintf("<!-- /iugo-ai:custom-agent:%s -->", extractName(marker))
...
// Example: "<!-- iugo-ai:custom-agent:my-skill -->" → "my-skill"
func extractName(marker string) string {
	prefix := "<!-- iugo-ai:custom-agent:"
	suffix := " -->"
