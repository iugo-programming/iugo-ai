...
func renderUpgradeSyncConfirm(results []update.UpdateResult, updateCheckDone bool, spinnerFrame int) string {
	...
	b.WriteString("\n")
	b.WriteString("     " + styles.SubtextStyle.Render("Updates iugo-ai, engram, and gga to latest versions"))
	b.WriteString("\n\n")
...
func renderUpgradeSyncResult(report *upgrade.UpgradeReport, syncFiles []string, upgradeErr error, syncErr error) string {
	...
	if reportUpgradedGentleAI(report) {
		b.WriteString("  " + styles.WarningStyle.Render("⚠ Sync skipped because iugo-ai was upgraded."))
		b.WriteString("\n")
		b.WriteString("  " + styles.SubtextStyle.Render("Restart iugo-ai, then run sync with the new binary."))
	} else if syncErr != nil {
