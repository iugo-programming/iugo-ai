...
import (
	...
	"github.com/iugo-programming/iugo-ai/internal/agentbuilder"
	"github.com/iugo-programming/iugo-ai/internal/backup"
	"github.com/iugo-programming/iugo-ai/internal/catalog"
	"github.com/iugo-programming/iugo-ai/internal/components/opencodeplugin"
...
func (m Model) confirmSelection() (tea.Model, tea.Cmd) {
	...
	case ScreenUpgrade:
		...
		// If iugo-ai itself was upgraded, leave the TUI so the app layer can restart
	...
	case ScreenProfiles:
		...
		case m.Cursor < profileCount:
			...
			// The ModelPicker shows iugo-orchestrator as the base row, so we need
		...
		if mode == model.UninstallModeFullRemove {
			...
			if isHomebrewManagedBinary(execPath) {
				result.ManualActions = append(result.ManualActions,
					"Homebrew-managed install detected. Run 'brew uninstall iugo-ai' to remove the executable cleanly.")
...
// Exception: if iugo-ai itself was upgraded, sync is skipped so the old
...
// agentsToManage returns the canonical list of agents iugo-ai should manage.
//
// Priority:
...
func (m Model) startInstallation() (tea.Model, tea.Cmd) {
	...
	return m, tea.Batch(tickCmd(), func() (msg tea.Msg) {
		...
		registryPath := filepath.Join(homeDir(), ".config", "iugo-ai", "custom-agents.json")
