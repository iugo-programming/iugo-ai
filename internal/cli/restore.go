...
// RunRestore is the top-level entry point for `iugo-ai restore [args]`.
...
func runRestoreWithHomeDir(args []string, restorer RestoreFunc, stdout io.Writer, stdin io.Reader, homeDir string) error {
	...
	if len(positional) == 0 {
		return fmt.Errorf("usage: iugo-ai restore [--list | latest | <id>] [--yes]")
	}
...
// and the iugo-ai version that created the backup when known.
...
func resolveRestoreTarget(target string, backups []backup.Manifest) (backup.Manifest, error) {
	...
	return backup.Manifest{}, fmt.Errorf("backup %q not found — use `iugo-ai restore --list` to see available backups", target)
}
