...
func TestDetectInstalledVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "iugo-ai uses build var",
			tool:         ToolInfo{Name: "iugo-ai", DetectCmd: nil},
			currentBuild: "1.5.0",
			wantVersion:  "1.5.0",
		},
		{
			name:         "iugo-ai dev build",
			tool:         ToolInfo{Name: "iugo-ai", DetectCmd: nil},
			currentBuild: "dev",
			wantVersion:  "dev",
		},
...
func TestFetchLatestRelease_GithubToken(t *testing.T) {
	...
	if gotUserAgent != "iugo-ai-update-check" {
		t.Fatalf("User-Agent = %q, want %q", gotUserAgent, "iugo-ai-update-check")
	}
}
...
func TestCheckAll(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		case contains(path, "iugo-ai"):
			release = githubRelease{TagName: "v1.5.0", HTMLURL: "https://github.com/IUGO-Programming/iugo-ai/releases/tag/v1.5.0"}
		case contains(path, "iugo-agent-guardian-angel"):
			release = githubRelease{TagName: "v2.0.0", HTMLURL: "https://github.com/IUGO-Programming/iugo-agent-guardian-angel/releases/tag/v2.0.0"}
	...
	// iugo-ai: 1.5.0 local == 1.5.0 remote → UpToDate
	assertResult(t, results[0], "iugo-ai", UpToDate, "1.5.0", "1.5.0")
...
func TestCheckAll_NetworkError(t *testing.T) {
	...
	// iugo-ai has no DetectCmd, so it gets currentBuildVersion "1.0.0" as local
	// but fetch fails → CheckFailed (it has a local version).
	if results[0].Status != CheckFailed {
		t.Fatalf("iugo-ai status = %q, want %q", results[0].Status, CheckFailed)
	}
	if results[0].Err == nil {
		t.Fatalf("iugo-ai expected error, got nil")
	}
...
func TestUpdateHint(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "iugo-ai macOS",
			tool:    ToolInfo{Name: "iugo-ai"},
			profile: system.PlatformProfile{OS: "darwin", PackageManager: "brew"},
			want:    "brew upgrade iugo-ai",
		},
		{
			name:    "iugo-ai linux",
			tool:    ToolInfo{Name: "iugo-ai"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "curl -fsSL https://raw.githubusercontent.com/IUGO-Programming/iugo-ai/main/scripts/install.sh | bash",
		},
		{
			name:    "iugo-ai windows",
			tool:    ToolInfo{Name: "iugo-ai"},
			profile: system.PlatformProfile{OS: "windows", PackageManager: "winget"},
			want:    "irm https://raw.githubusercontent.com/IUGO-Programming/iugo-ai/main/scripts/install.ps1 | iex",
		},
		{
		...
		},
		{
			...
			tool:    ToolInfo{Name: "engram"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "iugo-ai upgrade (downloads pre-built binary)",
		},
		{
			name:    "engram windows",
			tool:    ToolInfo{Name: "engram"},
		...
		},
		{
			...
			tool:    ToolInfo{Name: "gga"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "See https://github.com/IUGO-Programming/iugo-agent-guardian-angel",
		},
		{
			name:    "unknown tool",
...
func TestRegistryContents(t *testing.T) {
	...
	expected := map[string]struct {
		...
		"gga":                          {owner: "IUGO-Programming", repo: "iugo-agent-guardian-angel"},
	...
	// iugo-ai must have nil DetectCmd.
	if Tools[0].DetectCmd != nil {
		t.Fatalf("iugo-ai DetectCmd should be nil")
	}
...
func TestCheckAll_DevVersion(t *testing.T) {
	...
	// Override only the first tool (iugo-ai) by running CheckAll with "dev".
	origTools := Tools
	t.Cleanup(func() {
		httpClient = origClient
		lookPath = origLookPath
	...
	// Restrict to just iugo-ai to isolate the test.
	...
	if results[0].Status != DevBuild {
		t.Fatalf("iugo-ai dev status = %q, want %q", results[0].Status, DevBuild)
	}
}
...
// and that the dev-build sentinel causes iugo-ai to be reported as DevBuild.
...
// when the running iugo-ai binary reports version "dev", it is identified as a
// DevBuild and NOT reported as UpdateAvailable or VersionUnknown.
//
// The spec says:
//   - Dev build MUST be reported as development-build semantic
//   - iugo-ai self-upgrade is skipped while engram/gga remain eligible
func TestCheckFiltered_DevBuildSemanticsForGentleAI(t *testing.T) {
	...
	lookPath = func(string) (string, error) { return "", fmt.Errorf("not found") }
	execCommand = func(name string, args ...string) *exec.Cmd { return mockCmd("false") }
	Tools = []ToolInfo{Tools[0]} // iugo-ai only
	...
	r := results[0]
	if r.Tool.Name != "iugo-ai" {
		t.Fatalf("tool = %q, want iugo-ai", r.Tool.Name)
	}
...
// iugo-ai with "dev" version gets DevBuild while engram with a real version stays eligible.
func TestCheckFiltered_DevBuildSkipNotEligible(t *testing.T) {
	...
	// Only iugo-ai and engram for this test
	...
	// iugo-ai should be DevBuild
	if results[0].Status != DevBuild {
		t.Fatalf("iugo-ai status = %q, want DevBuild", results[0].Status)
	}
...
func TestNoUpdatesPath(t *testing.T) {
	...
	// Only engram and gga for this test (skip iugo-ai to avoid dev-build behavior)
