...
func TestExecute_NoopWhenNothingIsExecutable(t *testing.T) {
	results := []update.UpdateResult{
		makeResult("iugo-ai", update.UpToDate, "1.0.0", "1.0.0", update.InstallBinary),
...
// iugo-ai with DevBuild status must appear in Results as UpgradeSkipped
...
func TestExecute_DevBuildIsSkipped(t *testing.T) {
	...
	// iugo-ai (DevBuild) MUST appear as UpgradeSkipped with a ManualHint.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "iugo-ai" {
			r := report.Results[i]
			devResult = &r
		}
	}
	if devResult == nil {
		t.Fatalf("iugo-ai (DevBuild) must appear in Results — was not found")
	}
	if devResult.Status != UpgradeSkipped {
		t.Errorf("iugo-ai DevBuild Status = %q, want UpgradeSkipped", devResult.Status)
	}
	if devResult.ManualHint == "" {
		t.Errorf("iugo-ai DevBuild ManualHint must be non-empty")
	}
...
// A DevBuild tool (e.g. iugo-ai with version="dev") MUST appear in UpgradeReport.Results
...
func TestExecute_DevBuildSurfacedAsSkipped(t *testing.T) {
	...
	// iugo-ai (DevBuild) MUST appear in results as UpgradeSkipped.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "iugo-ai" {
			r := report.Results[i]
	...
	if devResult == nil {
		t.Fatalf("iugo-ai DevBuild must appear in Results as UpgradeSkipped, but was not found")
	}
	...
	if devResult.ManualHint == "" {
		t.Errorf("iugo-ai DevBuild ManualHint must be non-empty — should explain dev/source build")
	}
...
func TestConfigPathsForBackup_CoversManagedAgentPaths(t *testing.T) {
	...
	managedFiles := map[string]string{
		...
		".cursor/rules/iugo-ai.mdc":       "# Cursor rules",
	}
