...
func TestFindBinaryInTar(t *testing.T) {
	...
	entries := []struct {
		...
		{"iugo-ai_1.5.0_darwin_arm64/iugo-ai", content}, // binary in subdir
	}
	...
	outPath := filepath.Join(t.TempDir(), "iugo-ai")
	...
	err := downloadBinary(context.Background(), server.URL+"/release.tar.gz", "iugo-ai", outPath)
	if err != nil {
		t.Fatalf("downloadBinary: %v", err)
	}
...
func TestExpectedChecksumFor(t *testing.T) {
	content := "abc123  iugo-ai_1.0.0_darwin_arm64.tar.gz\ndef456  iugo-ai_1.0.0_linux_amd64.tar.gz\n"
	...
	tests := []struct {
	...
	}{
		{
			...
			content:  content,
			filename: "iugo-ai_1.0.0_darwin_arm64.tar.gz",
			want:     "abc123",
		},
		{
			name:     "found second entry",
			content:  content,
			filename: "iugo-ai_1.0.0_linux_amd64.tar.gz",
			want:     "def456",
		},
		{
			name:     "not found returns error",
			content:  content,
			filename: "iugo-ai_1.0.0_windows_amd64.zip",
			wantErr:  true,
		},
		{
...
func TestFetchChecksums(t *testing.T) {
	const fakeContent = "abc123  iugo-ai_1.0.0_darwin_arm64.tar.gz\n"
