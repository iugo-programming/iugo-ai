...
func antigravityEngramHooksJSON() []byte {
	cfg := map[string]any{
		"iugo-ai-engram-tools": map[string]any{
			"PreInvocation": []any{
				map[string]any{
					"type": "command",
...
func installAntigravityEngramPlugin(homeDir, engramCommand string) (bool, []string, error) {
	pluginDir := filepath.Join(homeDir, ".gemini", "antigravity-cli", "plugins", "iugo-ai-engram")
...
func injectWithOptions(configHomeDir, promptDir string, adapter agents.Adapter, opts InjectOptions) (InjectionResult, error) {
	...
	case model.StrategySeparateMCPFiles:
		...
		// `engram setup <agent>` is invoked. iugo-ai's Inject() runs after
		...
		// See: https://github.com/iugo-programming/iugo-ai/issues (engram absolute path regression)
	...
	case model.StrategyTOMLFile:
		...
		// Write iugo-ai SDD model-selection profile files into ~/.codex/.
...
// iugo-ai runs Inject() after setup, so we must not overwrite that absolute
