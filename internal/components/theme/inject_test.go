...
func TestInjectClaudeThemeWritesIUGOThemeFile(t *testing.T) {
	home := t.TempDir()
	...
	if root.Name != "IUGO" || root.Base != "dark" {
		t.Fatalf("theme identity = %q/%q, want IUGO/dark", root.Name, root.Base)
	}
