...
func TestBashScriptPathWindowsUsesForwardSlashes(t *testing.T) {
	profile := system.PlatformProfile{OS: "windows", PackageManager: "winget"}
	got := bashScriptPath(profile, `C:\Users\jorge\AppData\Local\Temp\iugo-guardian-angel\install.sh`)
	want := "C:/Users/jorge/AppData/Local/Temp/iugo-guardian-angel/install.sh"
	if got != want {
...
func TestResolveComponentInstall(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			component: model.ComponentGGA,
			want: CommandSequence{
				{"powershell", "-NoProfile", "-Command", fmt.Sprintf("Remove-Item -Recurse -Force -ErrorAction SilentlyContinue '%s'; exit 0", filepath.Join(os.TempDir(), "iugo-guardian-angel"))},
				{"git", "clone", "https://github.com/iugo-programming/iugo-guardian-angel.git", filepath.Join(os.TempDir(), "iugo-guardian-angel")},
				{gitBashPath(), bashScriptPath(system.PlatformProfile{OS: "windows"}, filepath.Join(os.TempDir(), "iugo-guardian-angel", "install.sh"))},
			},
		},
		{
