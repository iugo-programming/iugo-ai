...
func TestDefaultVariantsCachePath(t *testing.T) {
	...
	if !strings.HasSuffix(got, filepath.Join(".iugo-ai", "cache", "model-variants.json")) {
		t.Fatalf("expected path suffix .iugo-ai/cache/model-variants.json, got %q", got)
	}
	legacy := filepath.Join(".cache", "iugo-ai")
